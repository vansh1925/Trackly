...
function StatCard({ title, value, icon: Icon, trend, trendValue }) {
  return (
    <div className="bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-700 p-6 hover:shadow-md transition-shadow duration-200">
      <div className="flex items-start justify-between">
        ...
        {Icon && (
          <div className="ml-4 p-3 bg-slate-50 dark:bg-slate-800 rounded-lg">
            <Icon className="w-6 h-6 text-slate-600 dark:text-slate-300" strokeWidth={1.5} />
          </div>
        )}
      </div>
    </div>
